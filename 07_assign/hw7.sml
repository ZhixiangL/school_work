...
fun preprocess_prog e =
	case e of 
	  ...
	  | Intersect (x, y) => 
	  		(case (x,y) of
			  	(LineSegment _, LineSegment _) => Intersect (preprocess_prog x, preprocess_prog y)
			  | (LineSegment _, _) => Intersect (preprocess_prog x, y)
			  | (_, LineSegment _) => Intersect (x, preprocess_prog y)
			  | _ => e)
	  | Let (s, x, y) => 
	  		(case (x,y) of
			  	(LineSegment _, LineSegment _) => Let (s, preprocess_prog x, preprocess_prog y)
			  | (LineSegment _, _) => Let (s, preprocess_prog x, y)
			  | (_, LineSegment _) => Let (s, x, preprocess_prog y)
			  | _ => e)
	  | Shift (x, y, z) =>
	  		(case z of
			  	LineSegment _ => Shift (x, y, preprocess_prog z)
			  | _ => e)
	  | _ => e
