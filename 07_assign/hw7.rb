...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2)
      if @y1 < seg.y1
        aXstart, aXend, aYstart, aYend = @x1, @x2, @y1, @y2
        bXstart, bXend, bYstart, bYend = seg.x1, seg.x2, seg.y1, seg.y2
      else
        aXstart, aXend, aYstart, aYend = seg.x1, seg.x2, seg.y1, seg.y2
        bXstart, bXend, bYstart, bYend = @x1, @x2, @y1, @y2
      end
